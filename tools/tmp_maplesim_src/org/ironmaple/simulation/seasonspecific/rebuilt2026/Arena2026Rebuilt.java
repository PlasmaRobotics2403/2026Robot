...
public class Arena2026Rebuilt extends SimulatedArena {
  ...
  public static final class RebuiltFieldObstaclesMap extends FieldMap {
    ...
    public RebuiltFieldObstaclesMap(boolean AddRampCollider) {
      ...
      double trenchWallDistY =
          Inches.of(73.0).in(Meters) + Inches.of(47.0 / 2).in(Meters) + Inches.of(6).in(Meters);
   ...
   * change that use {@link #Arena2026Rebuilt(boolean)}. Additionally due to performance issues the
   * arena will not spawn all fuel by default. If you would like to change this use {@link
   ...
   * <h2>Creates an Arena for the 2026 FRC game rebuilt </h2>
   *
   * <p>Due to the nature of maple sim they can not be fully simulated and so either must be non
   * existent or treated as full colliders. This behavior can be changed with the AddRampCollider
   * variable. Additionally due to performance issues the arena will not spawn all fuel by default.
   * If you would like to change this use {@link #setEfficiencyMode(boolean)}
   *
   ...
   *
   * This method is useful for certain spawners like the return cutes on the hub to prevent the game
   * pieces from being returned to the exact same position every time.
   *
   ...
   *
   * When this is set too false both goals will always be set to active. Ths can be useful for
   * testing or too simulate endgame.
   *
   ...
   * player throws game pieces use {@link #outpostThrow(boolean, Rotation2d, Angle, LinearVelocity)}
   *
   ...
   * the hub does not have game pieces stored this method will not do anything. If you would like to
   * have the human player throw at the hub use {@link #outpostThrowForGoal(boolean)}
   *
   ...
   *
   * <p>Efficiency mode reduces the amount of game pieces on the field too increase performance.
